...
Inherit) live in `source/voice/` and are generated by `source/tts.py` with
[Piper TTS](https://github.com/OHF-Voice/piper1-gpl) and the free
`en_US-hfc_female-medium` voice — no license or attribution required. Use the script,
...
python3 source/tts.py voices/en_US-hfc_female-medium.onnx source/voice/
```
