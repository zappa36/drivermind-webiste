 ...
 *
 * 47.5s, 44.1kHz stereo, 120 BPM, keyed to the video's scene timeline:
 *   0–5s     title     — pad swell only
 *   5–12s    problem   — bass enters on bar starts
 *   12–36s   the loop  — kick pulse + plucked arpeggio with ping-pong echo
 *            (voiceover sits on top at ~13.4s and ~29.6s; ducking happens in mix.cjs)
 *   36–42s   benefits  — fullest section
 *   42–47.5s CTA       — pulses stop, chord resolves to C, fade out
 *
...
const CHORDS = [
  ...
  [32, 36, [A3, C4, E4, B4],      110.00, [A4, B4, C5, E5]],
  [36, 40, [F3, A3, C4, E4],      87.31,  [A4, C5, E5, F4]], // benefits
  [40, 42, [G3, B3, D4, E4],      98.00,  [B4, D5, E5, G5]], // push into the resolve
  [42, 47.5, [G3, C4, E4, D4],    130.81, [C5, D5, E5, G4]], // resolve on C
];
...
for (const [cs, ce, notes, root, arpSet] of CHORDS) {
  ...
  // bass: bar starts only during "problem", then a half-time pulse; long resolve at 42s
  if (cs >= 42) {
    bass(root, cs, 5.0, BASS_G * 1.1);
  } else {
    for (let t = cs; t < Math.min(ce, 42); t += 1.0) {
      if (t < 4) continue;
      if (t < 12 && t % 2 !== 0) continue; // sparse (bar starts) before the story begins
      bass(root, t, 0.92, BASS_G);
    }
  }
}

// kick pulse through the story + benefits (12–42s), half-time
for (let t = 12; t < 42; t += 1.0) kick(t, KICK_G);

// arpeggio: 3-3-2 rhythm in eighths (hits at 0, .75, 1.5 in each 2s bar), 12–42s
let arpIdx = 0;
for (let bar = 12; bar < 42; bar += 2) {
  for (const off of [0, 0.75, 1.5]) {
    const t = bar + off;
    if (t < 12 || t >= 42) continue;
    ...
    if (t >= 29) pluck(note / 2, t, ARP_G * 0.55, -pan);
    arpIdx++;
  }
}
...
for (let i = 0; i < N; i++) {
  ...
  if (t > 45.5) g *= Math.max(0, (DUR - t) / 2.0); // fade out
  L[i] *= g;
  R[i] *= g;
}
let peak = 0;
